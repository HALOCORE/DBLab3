...
if test_ctl['Customer']:
    ...
    # 参数错误查询
    printresp(requests.get("http://localhost:8000/api/v1/APICustomer/?wahaha=abc"))
    # 获取单个（存在）
...
if test_ctl['Staff']:
    ...
    printresp(requests.get("http://localhost:8000/api/v1/APIStaff/?staffName=小牛&staffAddress=合肥"))
    
    # 获取单个（存在）（不存在略）
    printresp(requests.get("http://localhost:8000/api/v1/APIStaff/221403199010127986"))
    # 创建单个
    ...
    # 删除单个（约束删不掉）（不存在略）
    printresp(requests.delete("http://localhost:8000/api/v1/APIStaff/221403199010127986"))
    
    # 一个Staff的Account
    printresp(requests.get("http://localhost:8000/api/v1/APIStaff/221403199010127986/CusAccount"))
    # 一个Staff的Loan
    printresp(requests.get("http://localhost:8000/api/v1/APIStaff/221403199010127986/Loan"))
...
if test_ctl['Branch']:
    ...
    # 参数错误查询
    printresp(requests.get("http://localhost:8000/api/v1/APIBranch/?wawawawa=AH"))
    # 查询某个支行
    ...
    # 删除某个支行(约束删不掉)
    printresp(requests.delete("http://localhost:8000/api/v1/APIBranch/AHB"))
    # 添加某个支行(约束加不了)
    printresp(requests.post("http://localhost:8000/api/v1/APIBranch/", {"city":"Hefei", "branchName":"AHB"}))
    # 更新某个支行
