...
def handle_deposit(request:HttpRequest):
    #按货币计算余额，储蓄账户数
    where = api__REQUEST.gen_where(request.GET)+" AND accountType='deposit'"
    remain_title, remain_data = api__REQUEST.calc_currency(where)
...
def handle_cheque(request:HttpRequest):
    #余额之和，支票账户数
    where = api__REQUEST.gen_where(request.GET)+" AND accountType='cheque'"
...
def handle_loan(request:HttpRequest):
    #贷款金额，发放金额，贷款记录数
    where = api__REQUEST.gen_where(request.GET)
