...
BEGIN
    ...
    values (var_accountIDX,staff_bran_branchName,var_staffID,var_remain,sysdate(),"deposit");
...
BEGIN
    ...
    select cust_customID into check_identity from cus_and_depAccount where cust_customID=var_customID and depo_cusA_accountIDX = var_accountIDX limit 1;
    select remain, openTime into account_remain, acc_opentime from cusAccount where accountIDX = var_accountIDX limit 1;
    select cusA_accountIDX into depAccount_record from depositAccount where cusA_accountIDX = var_accountIDX limit 1;
    ...
    if var_after_alter < 0.0 then
        SIGNAL SQLSTATE '45000'
		SET MESSAGE_TEXT = 'Error: Lack of balance.', MYSQL_ERRNO = 1001; -- not sure
    end if;
    update cusAccount set remain=var_after_alter, visitTime=sysdate() where accountIDX = var_accountIDX;
...
BEGIN
	...
	select cusA_accountIDX into depAccount_record from depositAccount where cusA_accountIDX = var_accountIDX;
